		...
		const int id)
{
	...
	if (id != -1) // Si un id est fourni, on l'utilise
	{
		p->id = id;
		usedIds.insert(id); // On l'ajoute à la liste des ids utilisés
	}
	else // Sinon, on génère un id unique
	{
		do
		{
			p->id = rand();
		} while (usedIds.count(p->id)); // Regénère tant que l'id est déjà utilisé
		usedIds.insert(p->id); // On l'ajoute à la liste des ids utilisés
	}

	return p; // Retourne l'adresse de la personne créée
}

void mariage(Person *p1, Person *p2)
{
	if (p1->partner != nullptr || p2->partner != nullptr) // Si l'un des deux est déjà marié
	{
		if (ancetre(p1, p2) == false || ancetre(p2, p1) == false || frereSoeur(p1, p2)) // Si ils sont ancêtres l'un de l'autre ou frère et sœur
		{
			return; // On ne fait rien
		}
	}
	else // Sinon on les "marie"
	{
		p1->partner = p2;
		p1->married = true;
		p2->partner = p1;
...
{
	return ((p1->father == p2->father) && (p1->mother == p2->mother)); // retourne true si même père et même mère
...
{
	return (p1->name == p2->name && p1->firstname == p2->firstname && p1->year_of_birth == p2->year_of_birth && p1->sex == p2->sex); // retourne true si même nom, prénom, année de naissance et sexe
...
{
	if (a == nullptr || b == nullptr) // cas de base
		return false;

	if (a->father == b || a->mother == b) // cas où b est un parent direct de a
		return true;
	// } else {
	...
	//   }
	// }
	return (ancetre(a->father, b) || ancetre(a->mother, b)); // cas récursif
}

int generations(Person *p)
{
	if (p == nullptr) // cas de base
		return 0;
	// 	int father = 1 + generations(p->father);
	// 	int mother = 1 + generations(p->mother);
	// 	return std::max(father, mother);
	return 1 + std::max(generations(p->father), generations(p->mother)); // version plus concise
}
...
{ // pareil que treeSize() mais avec showPerson()
	if (p == nullptr)
		return;
